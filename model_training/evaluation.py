...
from typing import List # Use a function definition from future version (say 3.x from 2.7 interpreter)
import requests
import os
import sys
...
model_file = "models/model-5.cntk"
			...
			for l in f:
				...
				for i in range(len(types)):
					if types[i] not in target_dict:
						types[i] = "$typing.Any$" #any zu Any gemacht
				# Run deep learner
				predictions = run_seq(tokens)
				if predictions == None or len(tokens) > minibatch_size:
					continue
				# Get stats
				for i in range(len(types)):
					if types[i] == "0":
					...
					top_5_pred_types = []
					for ix, (pr, _) in enumerate(predictions[i]):
						top_5_pred_types.append(target_wl[int(pr)])
						stop_k += 1
						if stop_k > 4:
							break
					print(types[i], dl_type, conf, dl_rank)
					f_out.write("%s\t%s\t%.4f\t%d\t%s\n" % (types[i], dl_type, conf, dl_rank, top_5_pred_types))
				f_out.flush()		
