...
for file in os.listdir(in_dir):
	...
	with open(in_dir + "/" + file, "r", encoding="utf-8") as f:
		...
		for ix, line in enumerate(content):
			...
			target_tokens = ["0"] + parts[1].split(' ') + ["0"] #O zu 0 geaendert
			...
			if len(source_tokens) != len(target_tokens):
				...
				break
			if len(source_tokens) > minibatchMaxSize or '\x0c' in source_tokens:
...
def write(file, sources, targets):
	with open(file, "w", encoding="utf-8") as f:
		...
		for i in range(len(sources)):
			...
			#TODO any zu typing.Any geaendert
			target_tokens = [token if token in target_word_vocab else '$typing.Any$' for token in target]
