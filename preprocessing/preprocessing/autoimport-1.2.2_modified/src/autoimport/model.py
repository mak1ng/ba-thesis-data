...
#datetime geaendert, MagicMock, -patch, _Call hinzugefuegt
common_statements: Dict[str, str] = {
    ...
    "_Call": "from unittest.mock import _Call",
    "_patch": "from unittest.mock import _patch",
...
class SourceCode:  # noqa: R090
    ...
    def _add_package(self, object_name: str) -> None:
        ...
        if import_string is not None:
            #added at beginning
            t = [loc for loc, val in enumerate(self.imports) if val.startswith("from __future__")]
            index = -1
            if t:
                index = max(t) 
            self.imports.insert(index + 1,import_string)
